...
function launch(num) {
    document.getElementById("page-container").setAttribute("style", "z-index: -1; position: relative;")   
...
for (var i=0, max=all.length; i < max; i++) {
    getNodesThatContain(all.item(i))
    if(all.item(i).className != "storyrocks-collection"){
    all.item(i).setAttribute("style", "z-index: 1 !important;")
    console.log(all.item(i))
}else if(all.item(i).className == "[class^=storyRocks]"){
    console.log("STORY ROCKS")
}
    
}

function getNodesThatContain(text) {
    var textNodes = $(document).find(":storyrocks-collection")
      .contents().filter( 
          function() {
           return this.nodeType == 3 
             && this.textContent.indexOf(text) > -1;
    });
    console.log(textNodes.parent());
